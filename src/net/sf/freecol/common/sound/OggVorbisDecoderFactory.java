...
public class OggVorbisDecoderFactory {
	...
	private static class OggStream extends InputStream {
		 ...
		 * @return Negative on error, zero on end of stream, otherwise the number of
		...
		/**
		 * Gets the OGG header (first three packets) which must contain vorbis audio
		 * content. This routine is public so it can be used as a check if the a file
		 ...
		 * Decode the PCM data.
		 *
		 * @return The number of bytes waiting in the conversion buffer to be written.
		 */
